...
@Service
public class BlogContentCRUDService {
    private static final String CONTENT_DIRECTORY = "content";
    public List<FilesHierarchy> getBlogTemplates(String blogId)  {
        File baseFolder = new File(BASE_FOLDER+blogId+File.separator+ CONTENT_DIRECTORY);
    ...
    public String getBlogTemplate(String blogId, String template) {
        File templateFile = new File(BASE_FOLDER+blogId+File.separator+ CONTENT_DIRECTORY +File.separator+template);
        System.out.println(template);
        try {
